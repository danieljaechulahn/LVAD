...
#This captures just the patients who were upgraded to Status 1 
upgradedtostatus1 <- full_data %>% filter(!is.na(list_time_for_status1)) %>% 
  left_join(just_form_hr_stat1 %>% select(PX_ID, JustId, CriteriaEcmoSupport, CriteriaBivadSupport, CriteriaMcsdSupport, VentricularEpisode), by = "PX_ID") %>%
  left_join(just_form_hr %>% select(JustId, Exception, listing_description, FormEffectiveDt), by = "JustId") %>%
  filter(date_of_first_status1 == FormEffectiveDt) %>%
  mutate(reason_for_status1 = case_when(
    CriteriaEcmoSupport == 1 ~ "ECMO",
    CriteriaBivadSupport == 1 ~ "BIVAD",
    Exception == 1 ~ "Exception",
    TRUE ~ "MCSD + Ventricular Arrhythmia"))

table(upgradedtostatus1$reason_for_status1)
  

#This calculates the median time it took to go from Status 2 to durable LVAD
...
sankey_plot <- ggplot(data_for_sankey, aes(x = x,                        
  ...
  theme(legend.position = 'none', 
        ...
        panel.grid = element_blank(),
        axis.text = element_text(size = 10)) + 
  labs(fill = 'Nodes', title = 'Outcomes within 6 Weeks after Receipt of Status 2 Following IABP/PEVAD Implantation') +
  scale_x_discrete(labels = c("Temporary MCS", "Durable LVAD", "Outcome within 6 Weeks"))
...
fulldata <- fulldata %>% left_join(cand_thor24 %>% select(PX_ID, CAN_LISTING_DT, CAN_REM_DT, CAN_REM_CD, CAN_INIT_ACT_STAT_CD, CAN_RACE, CAN_ETHNICITY_SRTR, CAN_GENDER, CAN_ABO, CAN_DGN, CAN_AGE_AT_LISTING, CAN_AGE_IN_MONTHS_AT_LISTING, CAN_BMI, CAN_PRIMARY_PAY, CAN_FUNCTN_STAT, CAN_PCW_MEAN, CAN_HGT_CM, CAN_WGT_KG, CAN_CARDIAC_OUTPUT, CAN_LISTING_CTR_CD), by = "PX_ID") %>%
...
pre_inotropes <- fortable1 %>% select(PX_ID, MCS, dLVAD, Outcome, date_of_first_status2, inotropes_before_mcs, high_dose_inotrope, multi_agent_low_dose_inotropes, mcs_with_hemo) %>% 
  mutate(
    qualifying_inotropes = ifelse(high_dose_inotrope == 1 | multi_agent_low_dose_inotropes == 1, 1, 0)) %>% 
  filter(qualifying_inotropes == 1) %>% left_join(just_form_hr_stat2 %>% select(PX_ID, McsdCardiacIndexInotropeSup, IabpCardiacIndexInotropeSup, JustId), by = "PX_ID") %>% left_join(just_form_hr %>% select(JustId, Exception, listing_description, FormEffectiveDt), by = "JustId") %>% group_by(PX_ID) %>% arrange(FormEffectiveDt) %>% slice(1) %>% mutate(
    measurements_with_tmcs = ifelse(McsdCardiacIndexInotropeSup == "Y" | IabpCardiacIndexInotropeSup == "Y", 1, 0)
  )
...
status_extensions <- just_form_hr_stat2 %>% left_join(just_form_hr %>% select(JustId, FormEffectiveDt, listing_description), by = "JustId") %>% 
  filter(listing_description == "Status 2 Extension")

status2_extension <- fortable1 %>% left_join(status_extensions, by = "PX_ID") %>%
  select(PX_ID, MCS, dLVAD, Outcome, unique_date, unique_event, date_of_first_status2, list_time, FormEffectiveDt, ChangeDt.y, listing_description) %>% 
  ...
  mutate(
    ...
    time_to_extension = as.numeric(difftime(extension_dt, date_of_first_status2, units = "days")),
    indexofextension = which(time_to_extension >= 0) [1],
    indexofextension = ifelse(is.na(indexofextension), 1, indexofextension),
    listing_description = case_when(
      time_to_extension > 0 & time_to_extension <= 14 & listing_description == "Status 2 Extension" ~ "Status 2 Extension",
      TRUE ~ NA)) %>% #this code calculates the time that has passed from date of obtaining Status 2 to when the Status 2 extension is activated. To allow for any delay in review of Status 2 extensions, we have allowed for review up to 21 days even though Status 2 expires after 14 days.
  filter(row_number() == indexofextension)
...
status2_extension <- status2_extension %>% 
  mutate(
    eligible_for_extension = case_when(
      Outcome == "Remains on Waitlist" | !is.na(FormEffectiveDt) | (Outcome != "Remains on Waitlist" & list_time > 14) ~ 1,
      TRUE ~ 0),
    listing_description = ifelse(listing_description == "Status 2 Extension" & is.na(FormEffectiveDt), NA, listing_description)
    )

lol <- status2_extension %>% filter(eligible_for_extension == 1)
...
table(fortable1$prelistingmcs)

#Identifies the patients who obtained Status 2 the same day they got an IABP or PEVAD
mcs_before_listing <- fortable1 %>% filter(prelistingmcs == 1) %>% mutate(
  time_from_listing_to_status2 = as.numeric(difftime(date_of_first_status2, CAN_LISTING_DT, units = "days"))
)
table(mcs_before_listing$MCS)
median(mcs_before_listing$time_from_mcs_to_status2)
median(fortable1$time_from_mcs_to_status2)
...
mcs_at_or_after_listing <- fortable1 %>% filter(prelistingmcs == 0) %>% 
  mutate(status_before_mcs = case_when(
    is.na(status_before_mcs) ~ "Not on Waitlist",
    TRUE ~ status_before_mcs))

#This code identifies the patients who were status 1 prior to MCS placement and why they were status 1 (all of them were on ECMO)
status1_before_mcs <- mcs_at_or_after_listing %>% filter(status_before_mcs == 1) %>%
  select(PX_ID, date_of_mcs, date_of_status_before_mcs) %>% left_join(just_form_hr_stat1 %>% select(PX_ID, JustId, CriteriaEcmoSupport, CriteriaBivadSupport, CriteriaMcsdSupport, VentricularEpisode), by = "PX_ID") %>%
  left_join(just_form_hr %>% select(JustId, Exception, listing_description, FormEffectiveDt), by = "JustId") %>%
  filter(FormEffectiveDt == date_of_status_before_mcs)



table(mcs_at_or_after_listing$status_before_mcs)
table(mcs_at_or_after_listing$status_before_mcs, mcs_at_or_after_listing$MCS)
table(mcs_at_or_after_listing$MCS)

median(mcs_at_or_after_listing$time_from_mcs_to_status2)
median(mcs_at_or_after_listing$listing_to_mcs_time)

mcs_at_listing <- mcs_at_or_after_listing %>% filter(listing_to_mcs_time == 0)

#Code to see the patients who were inactive or status 6 at the time of temporary MCS placement
status6_at_mcs <- mcs_at_or_after_listing %>% filter(status_before_mcs == 6 | status_before_mcs == "Inactive")
...
ggplot(data = mcs_at_or_after_listing, aes(x = status_before_mcs)) + 
...
both_mcs <- fortable1 %>% filter(post_iabp == 1 & post_impella == 1) %>%
  mutate(
    time_from_first_to_second = case_when(
      first_device == "iabp" ~ as.numeric(difftime(first_impella, first_iabp, units = "days")),
      first_device == "impella" ~ as.numeric(difftime(first_iabp, first_impella, units = "days"))))
...
median(iabp_then_impella$time_from_first_to_second)

impella_then_iabp <- both_mcs %>% filter(first_device == "impella")
median(impella_then_iabp$time_from_first_to_second)
...
remainsonwaitlist <- fortable1 %>% filter(Outcome == "Remains on Waitlist") %>% mutate(
  dLVAD = case_when(
    is.na(dLVAD) ~ "No Durable LVAD",
    TRUE ~ dLVAD
  )
)
  # group_by(status)

ggplot(data = remainsonwaitlist, aes(x = status)) + 
  geom_bar(aes(fill = dLVAD), color = "black", linewidth = 0.3, position = position_stack(reverse = TRUE)) +
  theme_classic() +
  theme(legend.title = element_blank()) +
  scale_fill_manual(name = "", values = c("#FA9E4D", "#b8a370")) +
  labs(x = "Status", y = "Number of Candidates") +
  theme(axis.text.x = element_text(size = 18),
        axis.title.x = element_text(size = 20),
        axis.text.y = element_text(size = 18),
...
table(remainsonwaitlist$dLVAD, remainsonwaitlist$status)
table(remainsonwaitlist$dLVAD)
...
caterpillarplot <- ggplot(receivedlvad, aes(x = center, fill = new_outcome)) +
...
rates <- receivedlvad %>% ungroup() %>% group_by(CAN_LISTING_CTR_CD) %>%
  summarize(
    ...
    prop_lvad_whole = sum(new_outcome == "Received Durable LVAD") / 174) %>% 
  arrange(-number_lvad) %>%
...
#This code stratifies all 5,288 patients by whether or not they got a durable LVAD
inotropes <- fortable1 %>%
  mutate(inotropic_support = case_when(
    high_dose_inotrope == 1 ~ "Single-Agent High-Dose Inotropic Support",
    multi_agent_low_dose_inotropes == 1 & high_dose_inotrope != 1 ~ "Multi-Agent Low-Dose Inotropic Support",
    inotropes_before_mcs == 1 & multi_agent_low_dose_inotropes != 1 & high_dose_inotrope != 1 ~ "Inotropic Support, Doses < Status 3 Criteria",
    TRUE ~ "No Inotropic Support"))
    

inotropes$inotropic_support <- factor(inotropes$inotropic_support, levels = c("No Inotropic Support", "Inotropic Support, Doses < Status 3 Criteria", "Multi-Agent Low-Dose Inotropic Support", "Single-Agent High-Dose Inotropic Support"))

number <- inotropes %>% group_by(CAN_LISTING_CTR_CD) %>%
  summarise(n = n())

inotropes <- inotropes %>% left_join(number) %>% ungroup() %>% group_by(CAN_LISTING_CTR_CD) %>% arrange(n, CAN_LISTING_CTR_CD)
  
groups = inotropes$CAN_LISTING_CTR_CD %>% unique
i = 0
inotropes$center = NA
for (g in groups) {
  i = i + 1
  inotropes[inotropes$CAN_LISTING_CTR_CD == g, ]$center = i
}

#Plots the per-center distribution of candidates, stratified by durable LVAD implantation rate
inotropesplot <- ggplot(inotropes, aes(x = center, fill = inotropic_support)) +
  geom_bar(position = "stack", stat = "count", width = 0.8) +
  theme_classic() +
  theme(legend.position = "bottom") + 
  theme(axis.text.x = element_text(size = 18),
...
inotrope_rates <- inotropes %>% ungroup %>% group_by(CAN_LISTING_CTR_CD) %>%
  summarize(
    number_of_patients = sum(n()),
    prop_ino_center = sum(inotropic_support == "Multi-Agent Low-Dose Inotropic Support" | inotropic_support == "High-Dose Inotropic Support") / n(),
    number_ino = sum(inotropic_support == "Multi-Agent Low-Dose Inotropic Support" | inotropic_support == "High-Dose Inotropic Support"),
    number_not_ino = sum(inotropic_support == "Inotropic Support, Doses < Status 3 Criteria" | inotropic_support == "No Inotropic Support"),
    prop_ino_whole = sum(inotropic_support == "Multi-Agent Low-Dose Inotropic Support" | inotropic_support == "High-Dose Inotropic Support") / 432) %>% 
  arrange(-number_ino) %>%
  mutate(Cum = cumsum(number_ino)) %>%
  mutate(CumProp = cumsum(prop_ino_whole))

median(inotrope_rates$prop_ino_center)

centers_wo_ino <- inotrope_rates %>% filter(number_ino == 0)
sum(inotrope_rates$number_of_patients)

chisqtable <- inotrope_rates %>% select(number_ino, number_not_ino)
